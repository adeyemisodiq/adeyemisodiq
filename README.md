...
UBI Advanced Stage project (96%). Fully automated Python exploit chain: unauthenticated OS command injection → SSH pivot around a `NoNewPrivileges` restriction → tar wildcard argument injection for root privilege escalation. 5/5 reliability across clean-snapshot runs, 10/10 automated pytest suite, verified negative retest against a patched target, and root-cause remediation (not payload filtering) for both vulnerabilities.
...
📫 Connect
[LinkedIn](https://linkedin.com/in/adeyemi-adegbite-3152882b0) · Ibadan, Nigeria
